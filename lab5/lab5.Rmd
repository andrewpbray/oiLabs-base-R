...
weights_nonsmoker = subset(nc$weight, nc$habit == "nonsmoker")
weights_smoker = subset(nc$weight, nc$habit == "smoker")

n_nonsmoker = length(weights_nonsmoker)
n_smoker = length(weights_smoker)

p = mean(weights_nonsmoker) - mean(weights_smoker)
se = sqrt((sd(weights_nonsmoker)^2 / n_nonsmoker) + (sd(weights_smoker)^2 / n_smoker))
Z = (p - 0) / se
...
First, we put all of the weights of the babies with non-smoking mothers and
smoking mothers into two separate vectors, `weights_nonsmoker` and 
`weights_smoker`. We calculate the length of those vectors and store them in
`n_nonsmoker` and `n_smoker`.
