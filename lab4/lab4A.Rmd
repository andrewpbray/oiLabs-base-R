...
But how can we compute, say, 5000 sample means, and put those results into a data frame to get a sampling distribution? We can use the `mosaic` function `do`:
...
The do function simply repeats a statement and collects the results in a data frame. If only one quantity is computed in each iteration, the default column name in the resulting data frame is `result`.

```{r head, eval=FALSE}
head(sample_means50)
```

If you would like to adjust the bin width of your histogram to show a little more detail, you can do so by changing the `nint` (for **n**umber of **int**ervals) argument.
