...
But how can we compute, say, 5000 sample means, and put those results into a data frame to get a sampling distribution? We can use the mosaic `do` function:
...
The do function simply repeats a statement and collects the results in a data frame, under the column "result".
...
Without the do function, this would be painful. First, we'd have to create an empty vector filled with 0s to hold the 5000 sample means. Then, we'd have to compute each of the 5000 sample means one line at a time, putting them individually into the slots of the `sample_means50` vector:
...
Behind the scenes, our code works like this:

**1.** Creates an empty data frame with a single column titled "results".

**2.** Takes a sample of size 50 and computes its mean.

**3.** Repeats step 2 5000 times, putting each sample mean into the next available slot in the "results" column of the data frame.

**4.** Puts the data frame inside the variable `sample_means50`.

If we look at the first few entires of `sample_means50` after we run the do function, we can see that our code did exactly this:

```{r head-do, eval=FALSE}
head(sample_means50)
```
