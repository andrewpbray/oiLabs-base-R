...
Using R, we're going to recreate many samples to learn more about how sample means and confidence intervals vary from one sample to another. *Loops* come in handy here. (If you are unfamiliar with loops, review Lab 4A at [http://www.openintro.org/stat/labs/04A_Intro_to_Statistical_Inference.pdf](http://www.openintro.org/stat/labs/04A_Intro_to_Statistical_Inference.pdf)).
...
* Use these statistics to calculate 50 confidence intervals.
...
require(mosaic)
n <- 60
samp <- do(50) * favstats(sample(population, size = n))
```

Lastly, we construct the confidence intervals.

```{r ci50, eval=FALSE}
samp = transform(samp, lower = mean - 1.96 * (sd / sqrt(n)))
samp = transform(samp, upper = mean + 1.96 * (sd / sqrt(n)))
```
...
with(samp, plot_ci(lower, upper, mean(population)))
```
