...
If you are familiar with computer programming, you know that a \hlsymbol{for} loop is a fundamental control-flow construct. However, you may find it easier to write loops using the \hlsymbol{mosaic} function \hlsymbol{do}, which simply repeats a statement and collects the results in a data frame. 
...
\paragraph{Nota Bene} The \hlsymbol{for} loop is an invaluable programming construction. However, everything that you need for this class can be done with \hlsymbol{do}. Please use whatever is most comfortable for you. 
...
Then add a new column \hlsymbol{sample.size} to the resulting data frame that indicates the sample size in each case. This new variable is simply $10$ repeated 5000 times, followed by $50$ repeated 5000 times, followed by $100$ repeated 5000 times. The use of the \hlsymbol{factor} function will ensure that \hlsymbol{R} considers this to be a categorical variable, and not a numerical one. 

<<eval=FALSE>>=
samp.dist$sample.size = factor(rep(c(10, 50, 100), each=5000))
@

Finally, draw the histogram using the \hlsymbol{|} formula notation. If you want to have the histograms stacked vertically rather than horizontally, use the \hlargument{layout} argument. 
