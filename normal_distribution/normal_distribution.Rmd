...
```{r hist-height}
ggplot(fdims, aes(x=hgt)) + geom_histogram(aes(y=..density..), binwidth=3)+stat_function(fun = dnorm, colour = "red", args=list(mean=fhgtmean, sd=fhgtsd))
```

This is a rather complicated `ggplot` expression. We start by specifying the dataset and the "aesthetics" (variables) of the plot. Because we are doing a plot of one variable, we only have to specify one variable here. Then, we use the "geom" `geom_histogram` like usual. The only change is that we use `..density..` as an argument to `y`, which tells `ggplot2` that we want to see a density histogram rather than a frequency histogram. Then, we add the `stat_function` and tell it we want a normal distribution with mean `fhgtmean` and standard deviation `fhgtsd`. 
...
```{r qq}
ggplot(fdims, aes(sample=hgt))+stat_qq()
```
