...
<<hist-height, eval=FALSE, message=FALSE>>=
...
Notice that adding the \hlsymbol{fit} argument to \hlfunctioncall{histogram} overlays a normal distribution on plot. The optional \hlsymbol{nint} argument simply specificies the number of bins. 

% %\symbolfootnote[2]{The top of the curve is cut off because the limits of the x- and y-axes are set to best fit the histogram.  To adjust the y-axis you can add a third argument to the histogram function: 
% 
% <<footnote, eval=FALSE>>=
