...
\emph{Hint:} Consider using functions like \hlsymbol{tally, table, sum} and/or \hlsymbol{length}.
...
Here is another way to perform the above simulations and compare the resulting histograms. Note that the code to generate the \hlsymbol{p\_hats} is exactly the same -- only the code to do the loop and draw the plots is different. 
