...
If you have not installed the packages already, run these commands:
...
The data are Arbuthnot baptism counts for boys and girls. You should see that the 
...
columns, just as it says next to the object in your environment. You can see the names of these columns (or 
variables) by typing:
...
to the data frame. What "total" contains is a list of 82 numbers, each one representing the sum we’re after. Take a look at a few of them and verify that they are right.
...
The special symbol `<-` performs an *assignment*, taking the output of a line of code or series of piped commands, and saving it into an object in your environment. In 
...
We can also pipe data into plots, as in
```{r piped-plot}
arbuthnot %>%
  mutate(prop = boys / (boys + girls)) %>%
  qplot(x = year, y = prop, data = ., geom = "line")
```

In this case, we had to use the `.` operator, which allows us to perform piped operations where the data is not always flowing into the first argument of a function. In this case, we have taken the arbuthnot data, then mutated it so it has an additional column, and then we want to create a plot. However, `qplot` expects to have data flowing in after the named `data=` argument. Because `data=` is not the first piece of the `qplot()` call, we have to tell R where to put the piped in data. The `.` does that. 

3.  Now, *mutate* the data frame once more to include a new column for the 
proportion of newborns that are girls, then generate a plot of that proportion 
over time. Remember, if you are going to use `qplot` as a standalone function you have to use assignment to save over your old version of the data frame. Otherwise, you can pipe together a series of data manipulations and then plot in one chain. 
What does the plot show you? 
...
This command returns a dataframe with an additional variable tacked on. The values of this variable are either `TRUE` if that year had more boys
