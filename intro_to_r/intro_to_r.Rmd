...
If you do not have the `oilabs` package installed, use the following command. 
```{r load-devtools-oilabs, eval=FALSE, message = FALSE}
devtools::install_github("andrewpbray/oilabs")
```

Then, load the packages and data. 
...
columns, just as it says next to the object in your workspace. You can see the names of these columns (or variables) by typing:
...
This command will show just the number of boys baptized each year.
...
```
Take a look at a few of the totals and verify that they are right. We'll be using this new vector to generate some plots, so we'll want to save it as a permanent column in our data frame.
