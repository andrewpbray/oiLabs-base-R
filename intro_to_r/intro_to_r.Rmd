...
This selects out of the `arbuthnot` data set only the number of boys baptized 
each year.
...
year, but there is a faster way. We can *mutate* the existing columns in the data
set to create a new one.

```{r calc-total-bapt-vars, eval=FALSE}
mutate(arbuthnot, total = boys + girls)
```

You'll see that there is a new column called "total" that has been tacked on to 
the data frame. Take a look at a few of the rows to verify that the sums are right. 

We'll be using this new column to generate some plots, so we'll need to be sure 
we make the mutation permanent.

```{r calc-total-bapt-vars-save, eval=FALSE}
arbuthnot <- mutate(arbuthnot, total = boys + girls)
```

The special symbol `<-` performs an *assignment*, taking the output of one line 
of code and saving it into an object in your workspace. In this case, you already
have an object called `arbuthnot`, so this command updates that data set with the
new mutated column.
...
arbuthnot <- mutate(arbuthnot, boys_to_girls_ratio = boys/girls)
```
...
arbuthnot <- mutate(arbuthnot, prop_boys = boys/total)
```
...
This command adds a column of values of either `TRUE` if that year had more boys
