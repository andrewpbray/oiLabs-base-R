...
\begin{flushleft}
\ttfamily\noindent
\hlsymbol{sample\usebox{\hlnormalsizeboxunderscore}mean}{\ }\hlassignement{\usebox{\hlnormalsizeboxlessthan}-}{\ }\hlfunctioncall{mean}\hlkeyword{(}\hlsymbol{sample}\hlkeyword{)}\mbox{}
\normalfont
\end{flushleft}
\end{kframe}
\end{knitrout}
...
\begin{flushleft}
\ttfamily\noindent
\hlsymbol{se}{\ }\hlassignement{\usebox{\hlnormalsizeboxlessthan}-}{\ }\hlfunctioncall{sd}\hlkeyword{(}\hlsymbol{sample}\hlkeyword{)}\hlkeyword{/}\hlfunctioncall{sqrt}\hlkeyword{(}\hlnumber{60}\hlkeyword{)}\hspace*{\fill}\\
...
\begin{exercise}
Does your confidence interval capture the true average size of houses in Ames? If you are working in this lab in a classroom, check to see if your neighbor's interval captures this value? 
\end{exercise}
...
\item Calculate 50 confidence intervals at this confidence the confidence level you chose in 2. You do not need to obtain new samples, simply calculate new intervals based on the sample means and standard deviations you have already collected. Using the \hlfunctioncall{plot\_ci} function, plot all intervals and calculate the proportion of intervals that include the true population mean. How does this percentage compare to the confidence level level selected for the intervals?
